...
class ActionOpportunityForm(forms.Form):
    kind = forms.ChoiceField(
        choices=[(key, label) for key, label in PostKind.choices if key != PostKind.GENERAL]
    )
    ...
    completion_instructions = forms.CharField(
...
class CommitmentForm(forms.Form):
    ...
    is_public = forms.BooleanField(
        required=False, initial=False, label="Show participation publicly"
    )
...
class InvitationForm(forms.Form):
    ...
    message = forms.CharField(
        required=False, widget=forms.Textarea(attrs={"rows": 2}), max_length=280
    )
