...
def validate_handle(value: str) -> str:
    ...
    if len(normalized) < settings.HANDLE_MIN_LENGTH:
        raise ValidationError(f"Handle must be at least {settings.HANDLE_MIN_LENGTH} characters.")
    if len(normalized) > settings.HANDLE_MAX_LENGTH:
        raise ValidationError(f"Handle must be at most {settings.HANDLE_MAX_LENGTH} characters.")
