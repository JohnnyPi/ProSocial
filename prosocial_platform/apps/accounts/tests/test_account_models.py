...
def test_duplicate_email_rejected():
    ...
    with pytest.raises(IntegrityError):
        User.objects.create_user(
            username="bob", email="alice@example.com", password="test-pass-123"
        )
...
def test_profile_created_for_new_user():
    user = User.objects.create_user(
        username="carol", email="carol@example.com", password="test-pass-123"
    )
...
def test_handle_case_insensitive_unique():
    user1 = User.objects.create_user(
        username="dave", email="d1@example.com", password="test-pass-123"
    )
    user2 = User.objects.create_user(
        username="eve", email="e2@example.com", password="test-pass-123"
    )
