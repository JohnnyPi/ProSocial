...
def reply_edit(request: HttpRequest, reply_id: uuid.UUID) -> HttpResponse:
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            update_reply(reply=reply, body=form.cleaned_data["body"])
            return redirect("posts:detail", public_id=reply.post.public_id)
    else:
        form = ReplyForm(instance=reply)
    return render(request, "interactions/reply_form.html", {"form": form, "post": reply.post, "reply": reply})
...
@login_required
def thank_post(request: HttpRequest, post_id: uuid.UUID) -> HttpResponse:
    post = get_post_for_display(public_id=post_id)
    if request.method != "POST":
        return redirect("posts:detail", public_id=post.public_id)
    try:
        added, _ = toggle_thank_you(sender=request.user, post=post)
    except (InteractionError, BlockedInteractionError):
        return HttpResponseForbidden("This interaction is not available.")
    if _is_htmx(request):
        from apps.interactions.models import ThankYou
        count = ThankYou.objects.filter(post=post).count()
        thanked = ThankYou.objects.filter(sender=request.user, post=post).exists()
        return render(
            request,
            "interactions/thank_you_button.html",
            {"post": post, "thank_count": count, "thanked": thanked},
        )
    return redirect("posts:detail", public_id=post.public_id)


@login_required
def thank_reply(request: HttpRequest, reply_id: uuid.UUID) -> HttpResponse:
    from apps.interactions.models import Reply, ThankYou
    reply = get_object_or_404(Reply.objects.visible(), public_id=reply_id)
    if request.method != "POST":
        return redirect("posts:detail", public_id=reply.post.public_id)
    try:
        toggle_thank_you(sender=request.user, reply=reply)
    except (InteractionError, BlockedInteractionError):
        return HttpResponseForbidden("This interaction is not available.")
    if _is_htmx(request):
        count = ThankYou.objects.filter(reply=reply).count()
        thanked = ThankYou.objects.filter(sender=request.user, reply=reply).exists()
        return render(
            request,
            "interactions/thank_you_button.html",
            {"reply": reply, "thank_count": count, "thanked": thanked},
        )
    return redirect("posts:detail", public_id=reply.post.public_id)
